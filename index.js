...
function manipulateUrl(url) {
  ...
  let param = objUrl.searchParams;
  param.append("newParams", "newValue");
  param.delete("oldParam");

  newUrl.href = objUrl.href;
  newUrl.protocol = objUrl.protocol;
  newUrl.host = objUrl.host;
  newUrl.pathname = objUrl.pathname;
  newUrl.search = objUrl.search;
  newUrl.params = [...param];
  return newUrl;
}
...
function checkURLParameters(url, params) {
  ...
  for (let value of params) {
    parObj[value] = urlObj.searchParams.has(value);
  }
  return parObj;
}
...
function getUrlData(url) {
  ...
  let urlObj = new URL(url);
  let newObj = {};
  newObj.origin = urlObj.origin;
  newObj.hostname = urlObj.hostname;
  newObj.port = urlObj.port;
  newObj.username = urlObj.username;
  newObj.password = urlObj.password;
  return newObj;
}
...
function sortUrlParams(url) {
  ...
  // Повертаємо новий URL як рядок.
  let urlObj = new URL(url);
  let par = [...urlObj.searchParams.entries()];
  par.sort();
  par.forEach(([key, value]) => urlObj.searchParams.delete(key));
  par.forEach(([key, value]) => urlObj.searchParams.append(key, value));
...
function getURLValues(url) {
  ...
  // Повертаємо масив значень пошукових параметрів.
  let urlObj = new URL(url);
  let URLSearchParams = urlObj.searchParams.keys();
  let par = [];
  for (let key of URLSearchParams) {
    par.push(...urlObj.searchParams.getAll(key));
  }
  return par;
}
